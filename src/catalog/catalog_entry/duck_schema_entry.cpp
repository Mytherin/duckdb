...
optional_ptr<CatalogEntry> DuckSchemaEntry::CreateTable(CatalogTransaction transaction, BoundCreateTableInfo &info) {
	...
	for (idx_t i = 0; i < fk_arrays.size(); i++) {
		...
		info.dependencies.AddDependency(*set.GetEntry(transaction, fk_info.name.name));
	}
...
void DuckSchemaEntry::DropEntry(ClientContext &context, DropInfo &info) {
	...
	auto existing_entry = set.GetEntry(transaction, info.name.name);
	if (!existing_entry) {
		throw InternalException("Failed to drop entry \"%s\" - entry could not be found", info.name.name);
	}
	if (existing_entry->type != info.type) {
		throw CatalogException("Existing object %s is of type %s, trying to drop type %s", info.name.name,
	...
	if (!set.DropEntry(transaction, info.name.name, info.cascade, info.allow_drop_internal)) {
