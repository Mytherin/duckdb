	...
	if (target.is_table_name || target.table_ref) {
		if (target.is_table_name) {
			...
			if (showref->show_type == ShowType::SHOW_FROM) {
				...
				if (IsInvalidSchema(base_table.GetSchemaName())) {
					showref->schema_name = base_table.GetTableName();
				} else {
					showref->catalog_name = base_table.GetSchemaName();
					showref->schema_name = base_table.GetTableName();
				}
			} else if (IsInvalidSchema(base_table.GetSchemaName())) {
				// Logic for unqualified relations (databases, tables, variables)
				auto table_name = StringUtil::Lower(base_table.GetTableName().GetIdentifierName());
		...
		if (showref->table_name.empty() && showref->show_type != ShowType::SHOW_FROM) {
			...
			if (target.is_table_name) {
				...
				table_ref->name.name = target.table_name;
