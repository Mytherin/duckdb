...
PEGTransformerFactory::TransformAlterTableStmt(PEGTransformer &transformer, const optional<bool> &if_exists,
	...
	result->SetCatalog(base_table_name->catalog_name);
	result->SetSchema(base_table_name->schema_name);
	result->name.name = base_table_name->table_name;
...
unique_ptr<AlterInfo> PEGTransformerFactory::TransformAlterSequenceStmt(PEGTransformer &transformer,
	...
	if (qualified_sequence_name.GetSchema().empty()) {
		alter_sequence_options->SetSchema(qualified_sequence_name.GetCatalog());
	} else {
		alter_sequence_options->SetCatalog(qualified_sequence_name.GetCatalog());
		alter_sequence_options->SetSchema(qualified_sequence_name.GetSchema());
	}
	alter_sequence_options->name.name = qualified_sequence_name.name;
...
QualifiedName PEGTransformerFactory::TransformQualifiedSequenceName(PEGTransformer &transformer,
	...
	return QualifiedName(catalog_qualification ? *catalog_qualification : INVALID_CATALOG,
...
PEGTransformerFactory::TransformSetSequenceOption(PEGTransformer &transformer,
	...
	for (auto &seq_option : sequence_option) {
		if (seq_option.first == "owned") {
			...
			auto schema = owned_by->qualified_name.GetSchema().empty() ? Identifier::DefaultSchema()
			                                                           : owned_by->qualified_name.GetSchema();
			owned_info =
