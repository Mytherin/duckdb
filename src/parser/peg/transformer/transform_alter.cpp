...
unique_ptr<AlterInfo> PEGTransformerFactory::TransformAlterSequenceStmt(PEGTransformer &transformer,
	...
	if (qualified_sequence_name.GetSchema().empty()) {
		alter_sequence_options->schema = qualified_sequence_name.GetCatalog();
	} else {
		alter_sequence_options->catalog = qualified_sequence_name.GetCatalog();
		alter_sequence_options->schema = qualified_sequence_name.GetSchema();
	}
