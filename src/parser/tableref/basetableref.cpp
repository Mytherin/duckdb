...
string BaseTableRef::ToString() const {
	string result;
	result += GetCatalogName().empty() ? "" : (SQLIdentifier(GetCatalogName()) + ".");
	result += GetSchemaName().empty() ? "" : (SQLIdentifier(GetSchemaName()) + ".");
	result += SQLIdentifier(GetTableName());
...
bool BaseTableRef::Equals(const TableRef &other_p) const {
	...
	return other.name == name && column_name_alias == other.column_name_alias &&
