...
BoundStatement Binder::BindAlterAddIndex(BoundStatement &result, CatalogEntry &entry,
	...
	auto bound_constraint = BindUniqueConstraint(*constraint_info.constraint, table_info.name.name, column_list);
	...
	create_index_info->table = table_info.name.name;
	...
	// Plan the table scan.
	TableDescription table_description(table_info.GetCatalog(), table_info.GetSchema(), table_info.name.name);
...
BoundStatement Binder::Bind(AlterStatement &stmt) {
	...
	if (stmt.info->type == AlterType::SET_COLUMN_COMMENT) {
		...
		EntryLookupInfo lookup_info(stmt.info->GetCatalogType(), QualifiedName(stmt.info->name.name));
		entry = entry_retriever.GetEntry(
		    EntryLookupInfo(lookup_info, QualifiedName(stmt.info->GetCatalog(), stmt.info->GetSchema(),
