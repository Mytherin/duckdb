...
unique_ptr<LogicalOperator> Binder::BindCopyDatabaseSchema(Catalog &from_database,
	...
	for (auto &entry : catalog_entries) {
		...
		create_info->name.SetCatalog(target_database_name);
...
unique_ptr<LogicalOperator> Binder::BindCopyDatabaseData(Catalog &source_catalog,
	...
	for (auto &table_ref : entries.tables) {
		...
		insert_node.table.SetCatalog(target_database_name);
		insert_node.table.SetSchema(table.ParentSchema().name);
		insert_node.table.name = table.name;

		auto from_tbl = make_uniq<BaseTableRef>();
		from_tbl->name = QualifiedName(source_catalog.GetName(), table.ParentSchema().name, table.name);
