...
BoundStatement Binder::Bind(BaseTableRef &ref) {
	...
	EntryLookupInfo table_lookup(CatalogType::TABLE_ENTRY, QualifiedName(ref.GetTableName()), entry_at_clause,
	                             error_context);
	BindSchemaOrCatalog(entry_retriever, ref.name);
	auto table_or_view =
	    entry_retriever.GetEntry(EntryLookupInfo(table_lookup, QualifiedName(ref.GetCatalogName(), ref.GetSchemaName(),
	...
	if (!table_or_view) {
		...
		table_or_view = entry_retriever.GetEntry(
		    EntryLookupInfo(table_lookup, QualifiedName(ref.GetCatalogName(), ref.GetSchemaName(),
		                                                table_lookup.GetEntryIdentifier())),
