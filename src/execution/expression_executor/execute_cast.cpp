...
unique_ptr<ExpressionState> ExpressionExecutor::InitializeState(const BoundCastExpression &expr,
	...
	if (expr.GetBoundCastInfo().HasInitLocalState()) {
		auto context_ptr = root.executor->HasContext() ? &root.executor->GetContext() : nullptr;
		CastLocalStateParameters parameters(context_ptr, expr.GetBoundCastInfo().GetCastData());
		result->local_state = expr.GetBoundCastInfo().InitLocalState(parameters);
	}
...
void ExpressionExecutor::Execute(const BoundCastExpression &expr, ExpressionState *state, const SelectionVector *sel,
	...
	CastParameters parameters(expr.GetBoundCastInfo().GetCastData(), false, error_ref, lstate);
	...
	expr.GetBoundCastInfo().Cast(child, result, cast_count, parameters);
	if (all_constant) {
