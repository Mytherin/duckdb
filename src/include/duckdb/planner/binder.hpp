...
private:
	...
	void BindDefaultValues(const SchemaCatalogEntry &schema, const ColumnList &columns, vector<unique_ptr<Expression>> &bound_defaults);
	...
	BoundLimitNode BindLimitValue(OrderBinder &order_binder, unique_ptr<ParsedExpression> limit_val, bool is_percentage,
	                              bool is_offset);
	static vector<CatalogSearchEntry> GetSchemaSearchPath(const SchemaCatalogEntry &schema);
