...
public:
	...
	LocalTableStorage(ClientContext &context, DataTable &table, LocalTableStorage &parent, idx_t changed_idx,
	                  const LogicalType &target_type, const vector<PhysicalIndex> &bound_columns, Expression &cast_expr);
...
public:
	...
	//! Scan
	void Scan(CollectionScanState &state, const vector<PhysicalIndex> &column_ids, DataChunk &result);
	...
	void FetchChunk(DataTable &table, Vector &row_ids, idx_t count, const vector<PhysicalIndex> &col_ids,
