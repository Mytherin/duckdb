...
buffer_ptr<VectorBuffer> SequenceBuffer::Flatten(const LogicalType &type, const SelectionVector &sel,
	...
	if (!sel.IsSet()) {
		...
		count = MaxValue<idx_t>(size.GetIndex(), count);
	}
...
void SequenceVector::GetSequence(const Vector &vector, int64_t &start, int64_t &increment) {
	if (vector.GetVectorType() != VectorType::SEQUENCE_VECTOR) {
		throw InternalException("SequenceVector::GetSequence called but vector is not a sequence vector");
	}
