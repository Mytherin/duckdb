...
void Vector::Reinterpret(const Vector &other) {
	...
	if (GetVectorType() == VectorType::DICTIONARY_VECTOR && other_type != this_type) {
		...
		buffer = make_buffer<DictionaryBuffer>(old_dict.GetSelVector(), old_dict.Size(), std::move(new_entry));
...
void Vector::Dictionary(buffer_ptr<DictionaryEntry> reusable_dict, const SelectionVector &sel, idx_t count) {
	if (type.InternalType() == PhysicalType::STRUCT) {
		throw InternalException("Struct vectors cannot be dictionaries");
	}
	D_ASSERT(type == reusable_dict->data.GetType());
	buffer = make_buffer<DictionaryBuffer>(sel, count, std::move(reusable_dict));
}
