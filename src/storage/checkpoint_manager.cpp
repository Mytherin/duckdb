...
void CheckpointReader::ReadTrigger(CatalogTransaction transaction, Deserializer &deserializer) {
	...
	auto &schema = catalog.GetSchema(transaction, trigger_info.GetSchema());
	auto table_entry = schema.GetEntry(transaction, CatalogType::TABLE_ENTRY, trigger_info.base_table->GetTableName());
	if (!table_entry) {
...
void CheckpointReader::ReadIndex(CatalogTransaction transaction, Deserializer &deserializer) {
	...
	auto &schema = catalog.GetSchema(transaction, create_info->GetSchema());
...
void CheckpointReader::ReadTable(CatalogTransaction transaction, Deserializer &deserializer) {
	...
	auto &schema = catalog.GetSchema(transaction, info->GetSchema());
