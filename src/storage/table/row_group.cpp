...
void CollectionScanState::Initialize(const vector<LogicalType> &types) {
	...
	for (idx_t i = 0; i < column_ids.size(); i++) {
		...
		column_scans[i].Initialize(types[col_id]);
	}
}
...
bool RowGroup::InitializeScanWithOffset(CollectionScanState &state, idx_t vector_offset) {
	...
	for (idx_t i = 0; i < column_ids.size(); i++) {
		auto column = column_ids[i].GetPrimaryIndex();
...
bool RowGroup::CheckZonemap(TableFilterSet &filters, const vector<PhysicalIndex> &column_ids) {
	for (auto &entry : filters.filters) {
		auto column_index = entry.first;
		auto &filter = entry.second;
		const auto &base_column_index = column_ids[column_index].GetPrimaryIndex();
...
bool RowGroup::CheckZonemapSegments(CollectionScanState &state) {
	...
	for (auto &entry : filters->filters) {
		...
		auto base_column_idx = column_ids[column_idx].GetPrimaryIndex();
		...
		if (count == max_count && !table_filters) {
			...
			if (table_filters) {
				...
				for (idx_t i = 0; i < table_filters->filters.size(); i++) {
					...
					auto col_idx = column_ids[tf_idx].GetPrimaryIndex();
...
void RowGroup::FetchRow(TransactionData transaction, ColumnFetchState &state, const vector<PhysicalIndex> &column_ids,
                        row_t row_id, DataChunk &result, idx_t result_idx) {
	for (idx_t col_idx = 0; col_idx < column_ids.size(); col_idx++) {
		auto column = column_ids[col_idx].GetPrimaryIndex();
		if (column == COLUMN_IDENTIFIER_ROW_ID) {
...
void RowGroup::UpdateColumn(TransactionData transaction, DataChunk &updates, Vector &row_ids,
                            const PhysicalIndex &column_index) {
